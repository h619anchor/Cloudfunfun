...
// ▶ Filter 클래스는 Front Controller 앞에 위치하여 실행(WAS에 의해 관리)되며 Interceptor 클래스는 Front Controller 뒤에 위치하여 실행(스프링 컨테이너에 의해 관리)

// 관리자 관련 권한 처리를 위해 작성된 인터셉터 클래스
// ▶ 요청 처리 메소드가 호출 전에 비로그인 사용자이거나 관리자가 아닌 사용자가 페이지를 요청한 경우 인위적으로 예외 발생 - 에러 페이지로 응답 처리
public class AdminAuthInterceptor implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		if (loginAccount == null || loginAccount.getStatus() != 0) {
